...
from thermosim.utils import kelvin_to_celsius


def sweep_boiler_pressure(
    P_range, T_superheat=773.15, P_condenser=10e3, eta_pump=0.85, eta_turbine=0.87,
):
    n = len(P_range)
    eta = np.zeros(n)
    w_net = np.zeros(n)
    ...
    for i, P in enumerate(P_range):
        try:
            result = RankineCycle(
                fluid="Water", P_boiler=P, T_superheat=T_superheat,
                P_condenser=P_condenser, eta_pump=eta_pump, eta_turbine=eta_turbine,
            ).solve()
...
def sweep_2d(P_range, T_range, P_condenser=10e3, eta_pump=0.85, eta_turbine=0.87):
    ...
    for i, P in enumerate(P_range):
        for j, T in enumerate(T_range):
            try:
                result = RankineCycle(
                    fluid="Water", P_boiler=P, T_superheat=T,
                    P_condenser=P_condenser, eta_pump=eta_pump, eta_turbine=eta_turbine,
                ).solve()
...
def main():
    ...
    apply_style(
        ax1, title="Thermal Efficiency vs. Boiler Pressure",
    ...
    apply_style(
        ax2, title="Net Work Output vs. Boiler Pressure",
        xlabel="Boiler Pressure [MPa]", ylabel="Net Work [kJ/kg]",
    )
    fig.tight_layout()
    ...
    apply_style(
        ax, title="Rankine Cycle Efficiency Map",
        xlabel="Superheat Temperature [C]", ylabel="Boiler Pressure [MPa]",
    )
    fig.tight_layout()
