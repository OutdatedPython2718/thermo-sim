...
from thermosim.utils import kelvin_to_celsius, pa_to_mpa, j_to_kj
...
def main():
    ...
    for label, st in zip(labels, result.states):
        print(f"  {label}: T={kelvin_to_celsius(st.T):.1f}C, P={pa_to_mpa(st.P):.3f} MPa, h={j_to_kj(st.h):.1f} kJ/kg, s={st.s / 1e3:.4f} kJ/(kg·K)")
